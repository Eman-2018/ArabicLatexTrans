...
def add_arabic_package(latex_code):
    if "\\usepackage{polyglossia}" not in latex_code and "\\usepackage{babel}" not in latex_code:
        ...
        if begin_doc_match:
            # Insert right before \begin{document}, NOT right after
            # \documentclass. bidi (loaded internally by polyglossia for
            # Arabic) requires essentially every other package to load
            # before it -- inserting here, after acl.sty and everything it
            # pulls in (times, hyperref, xcolor, natbib, etc.), avoids the
